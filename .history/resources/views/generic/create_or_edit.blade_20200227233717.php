                    ...
                    @foreach ($entries as $entry)
                        ...
                        @dump($loop->parent->index)
                            <div class="row my-2">
                                ...
                                <div class="row col">
                                    <input id="entry-{{ ($entry->id ?? $loop->parent->index) }}-{{ $field }}" name="entry[{{ ($entry->id ?? $loop->parent->index) }}][{{ $field }}]" value="{{ old("entry[{($entry->id ?? $loop->index)}][{$field}]") != "" ? old("entry[{($entry->id ?? $loop->index)}][{$field}]") : $entry->$field }}" class="form-control row" />@error($field)
