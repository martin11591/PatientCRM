                ...
                <div class="card-body">
                    ...
                    @foreach ($entries as $entry)
                        @foreach ($fields as $field)
                            <div class="row my-2">
                                <label class="mb-0 col-md-5 col-lg-4 font-weight-bold" for="{{ $field }}">{{ __("layout.{$title}_$field") }}: </label>
                                <div class="row col">
                                    <input id="{{ $field }}" name="{{ $field }}" value="{{ old($field) != "" ? old($field) : $entry->$field }}" class="form-control row" />@error($field)
