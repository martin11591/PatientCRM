...
@section('content_header')
    <h1 class="m-0 text-dark">{{ trans_choice("layout.edit_{$title}", $amount ?? 1) }}</h1>
@endsection
                    ...
                    @for ($index = 0; $index < $amount; $index++)
                        @foreach ($fields as $field)
                            <div class="row my-2">
                                ...
                                <div class="row col">
                                    <input id="entry-{{ $entry->id }}-{{ $field }}" name="entry[{{ $entry->id }}][{{ $field }}]" value="{{ old("entry[{$entry->id}][{$field}]") ?? "" }}" class="form-control row" />@error($field)
