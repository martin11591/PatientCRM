...
@section('content')
    <table id="diseases_table" class="table table-bordered table-hover">
            ...
            <tr>
                <th scope="col" data-orderable="false" data-searchable="false">
                    <form action="" method="POST" id="form_multi" name="multi">
                        <input type="checkbox" name="id" form="form_multi" value="@foreach ($diseases as $disease) {{ (!$loop->first ? ',' : '') . intval($disease->id) }} @endforeach">
                    </form>
                </th>
        ...
        @foreach ($diseases as $disease)
            <tr>
                <td>
                    <input type="checkbox" name="id[]" form="form_multi">
                </td>
                <td>{{ $disease->name }}</td>
