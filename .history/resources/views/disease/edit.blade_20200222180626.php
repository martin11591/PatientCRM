                    ...
                    @foreach ($entries as $id => $entry)
                        @foreach ($fields as $field)
                            ...
                            <div class="row my-2">
                                ...
                                @enderror
                                @else
                                    @foreach ($field['groups'] as $fieldGroup)
                                <label class="mb-0 col-md-5 col-lg-4 font-weight-bold" for="{{ $fieldGroup }}">{{ __("layout.disease_$fieldGroup") }}: </label>
                                <div class="row col">
                                    <input id="entry-{{ $id }}-{{ $fieldGroup }}" name="entry[{{ $id }}][{{ $fieldGroup }}]" value="{{ old($fieldGroup) != "" ? old($fieldGroup) : $entry->$fieldGroup }}" class="form-control row" />@error($fieldGroup)
