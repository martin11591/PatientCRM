        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    @foreach ($entries as $id => $entry)
                        ...
                        @foreach ($fields as $key => $field)
                            <div class="row my-2">
                                ...
                                <label class="mb-0 col-md-5 col-lg-4 font-weight-bold" for="entry-{{ $id }}-{{ $field }}">{{ __("layout.medicine_$field") }}: </label>
                                <div class="row col">
                                    <input id="entry-{{ $id }}-{{ $field }}" name="entry[{{ $id }}][{{ $field }}]" form="medicineUpdate" value="{{ old("entry[{$id}][{$field}]") != "" ? old("entry[{$id}][{$field}]") : $entry->$field }}" class="form-control row" />@error($field)
                                    <p class="row text-danger">{{ $errors->first($field) }}</p>
                                @enderror
                                @else
                                    @foreach ($field as $fieldGroup)
                                <label class="mb-0 col-md-5 col-lg-4 font-weight-bold" for="entry-{{ $id }}-{{ $key }}-{{ $fieldGroup }}">{{ __("layout.medicine_{$key}") }}: </label>
                                <div class="row col">
                                    @if (isset($entry->$relation))
                                        @foreach ($entry->$relation as $groupKey => $groupValue)
                                    <div class="d-flex flex-nowrap row mb-2 w-100">
                                        <select{{-- id="entry-{{ $id }}-{{ $key }}-{{ $fieldGroup }}"--}} name="entry[{{ $id }}][{{ $key }}][]" class="form-control d-inline-block" form="medicineUpdate">
                                            <option value=""></option>
                    ...
                    @endforeach
                        <div class="row">
                            <a href="{{ route('medicine.index', 'post') }}" class="btn btn-secondary ml-auto mt-4">{{ __('layout.back') }}</a>
                            <form id="medicineUpdate" method="POST" action="{{ route('medicine.update', 'post') }}">
                            @csrf
                            @method('PUT')
