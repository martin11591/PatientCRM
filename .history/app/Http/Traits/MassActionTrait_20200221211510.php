...
trait MassActionTrait {
    ...
    private function createMessage($processResults, $texts = []) {
        ...
        if (isset($processResults['success']) && $processResults['success'] > 0) array_push($message, trans_choice($message['success'], $processResults['success'], ['count' => $processResults['success']]));

        if (isset($processResults['fail']) && $processResults['fail'] > 0) array_push($message, trans_choice($message['fail'], $processResults['fail'], ['count' => $processResults['fail']]));
        ...
        if (isset($notFound) && $notFound > 0) array_push($message, trans_choice($message['fail'], $notFound, ['count' => $notFound]));
