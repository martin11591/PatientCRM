...
trait MassActionTrait {
    ...
    private function createMessage($processResults, $texts = []) {
        ...
        $texts = array_merge([
            'not_found' => 'layout.items_not_found',
            'fail' => 'layout.items_action_error',
            'success' => 'layout.items_action_success',
        ], $texts);
        $messages = [];

        if (isset($processResults['success']) && $processResults['success'] > 0) array_push($messages, trans_choice($texts['success'], $processResults['success'], ['count' => $processResults['success']]));

        if (isset($processResults['fail']) && $processResults['fail'] > 0) array_push($messages, trans_choice($texts['fail'], $processResults['fail'], ['count' => $processResults['fail']]));
        ...
        if ($notFound > 0) array_push($messages, trans_choice($texts['not_found'], $notFound, ['count' => $notFound]));
