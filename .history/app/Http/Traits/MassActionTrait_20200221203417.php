...
trait MassActionTrait {
    ...
    private function createMessage($processResults, $texts) {
        ...
        if (isset($processResults->succeeded) && $processResults->succeeded > 0) {
            if ($processResults->succeeded == 1) array_push($message, __($texts['layout.deleted_success']));
            else array_push($message, __($texts['layout.deleted_success_many'], ['count' => $processResults->succeeded]));
        }
        if (isset($processResults->failed) && $processResults->failed > 0) {
            if ($processResults->failed == 1) array_push($message, __($texts['layout.delete_error']));
            else array_push($message, __($texts['layout.delete_error_many'], ['count' => $processResults->failed]));
        }
        ...
        if (isset($processResults->entriesAmount)) {
            ...
            if ($notFoundCount == 1) array_push($message, __($texts['layout.not_found']));
            else if ($notFoundCount > 1) array_push($message, __($texts['layout.not_found_many'], ['count' => $notFoundCount]));
        }
