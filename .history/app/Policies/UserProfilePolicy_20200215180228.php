...
class UserProfilePolicy
{
    ...
    /**
     * Return names for checking and to use in template
     */
    private function getGroupsNames($groups) {
        ...
        foreach ($groups as $group) {
             ...
             * We change it from number to bit number
             */
            $names .= ($names != "" ? ", ": "") . trans("groups.{$group->name}");
        }
        return $names;
    }
    /**
    ...
    public function view(?User $user, UserProfile $userProfile)
    {
         ...
         */
        dd((!($this->isUserInGroup($user, self::PATIENT))),
            $this->isUserInGroup($user, self::RECEPTIONIST),
            $this->isUserInGroup($user, self::DOCTOR),
            $this->isUserInGroup($user, self::SUPERUSER),
            (!($this->isUserInGroup($user, self::PATIENT))) || ($this->isUserInGroup($user, self::RECEPTIONIST)) || ($this->isUserInGroup($user, self::DOCTOR)) || ($this->isUserInGroup($user, self::SUPERUSER))
        );
